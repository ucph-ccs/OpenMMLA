...
to delete them, and how long they are kept.
...
HELP = (
    ...
    "Start, filed under the day it started. The file a stream is writing now is never deleted, as that frees "
    "nothing until the stream stops and loses the take: Stop it first. A dim name is a stream that is not in "
    "this card's Streams (another card's, or one removed since).\n"
    "Keep recordings for sets record_keep_days of every stream here (Config tab): a recording last written "
    "longer ago is deleted at its stream's next Start and at Refresh on the Streams tab. Copies fetched with "
    "Download are on this machine under artifacts/streams-<date>/ (Sessions → Delete Artifacts); the Stream "
    "Server's own recordings are on its card."
)
...
class StreamRecordingsScreen(ModalScreen):
    ...
    def _confirm_or_run(self, key: tuple, plan: list[cr.CaptureFile], button: str, what: str) -> None:
        ...
        if self._pending != key:
            self._pending = key
            self._set_log(
                f"[red]This deletes {what} from the capture host's disk, for good: Download has to have copied "
                f"what is still wanted. Press {button} again to confirm.[/red]")
            return
        self._pending = None
        self._deleting = True
...
def describe(results: list[cr.Deletion]) -> str:
    ...
    for result in results:
        ...
        if result.live:
            lines.append(f"[yellow]Left on {where}, being recorded now: {len(result.live)}.[/yellow]")
        if result.failed:
