...
class SpeechSeparator(Server):
    ...
    def _setup_yaml(self):
        ...
        self.cuda = self.config['SpeechSeparator'].get('cuda', True)
        self.cuda = self.cuda and torch.cuda.is_available()
        self.device = 'gpu' if self.cuda else 'cpu'
        self.sp_model = self.config['SpeechSeparator']['model']
        self.sp_model_local = self.config['SpeechSeparator']['model_local']

    def _setup_objects(self):
        """Load the speech separation model."""
        try:
            self.separation_model = pipeline(Tasks.speech_separation, device=self.device, model=self.sp_model)
        except ValueError:
