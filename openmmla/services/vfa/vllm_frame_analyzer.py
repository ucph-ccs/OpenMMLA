...
class VLLMFrameAnalyzer(Server):
    ...
    def _setup_yaml(self):
        self.families = self.config['VLLMFrameAnalyzer']['families']
        self.backend = self.config['VLLMFrameAnalyzer']['backend']
        self.top_p = float(self.config['VLLMFrameAnalyzer']['top_p'])
        self.temperature = float(self.config['VLLMFrameAnalyzer']['temperature'])
        self.end_to_end = self.config['VLLMFrameAnalyzer'].get('end_to_end', False)

        if self.backend in ['ollama', 'vllm', 'openai', 'qwen', 'gemini', 'deepseek']:
            backend_config = self.config['VLLMFrameAnalyzer'][self.backend]
        else:
        ...
        self.action_definitions = '\n'.join(
            [f"'{key}': {value}" for key, value in self.config['VLLMFrameAnalyzer']['defined_actions'].items()])
    ...
    def process_request(self):
        ...
        try:
            ...
            if self.end_to_end:
                ...
                print(f'vlm_result_obj: {vlm_result_obj}')
                ...
                print(f'llm_result_obj: {llm_result_obj}')
