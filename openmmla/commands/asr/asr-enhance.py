...
def get_app():
    ...
    project_dir = os.environ.get("SPEECH_ENHANCER_PROJECT_DIR")
    config_path = os.environ.get("SPEECH_ENHANCER_CONFIG_PATH")

    if not project_dir:
        print("WARNINGS: Environment variable SPEECH_ENHANCER_PROJECT_DIR not set. Using current working directory.")

    if not config_path:
        raise RuntimeError(
            "Environment variables SPEECH_ENHANCER_CONFIG_PATH must be set."
        )
...
def main():
    ...
    os.environ["SPEECH_ENHANCER_PROJECT_DIR"] = args.project_dir if args.project_dir else os.getcwd()
    os.environ["SPEECH_ENHANCER_CONFIG_PATH"] = args.config_path
