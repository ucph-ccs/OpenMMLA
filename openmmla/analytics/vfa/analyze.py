...
def vfa_session_analysis(project_dir, session_id, influx_client):
    ...
    action_data = fetch_and_process_data(session_id, EVENT_TYPE_VFA_ACTION, influx_client)
    action_json_file_path = save_to_json_file(session_id, action_data, "action_recognition", log_dir)

    print(f"Action Recognition session analysis completed for session: {session_id}")
