...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Start speech separation server.",
    ...
    add_arg('port', int, 5004, 'port number to bind the server', shortname='-P')
    return parser
...
def get_app():
    ...
    project_dir = os.environ.get("SPEECH_SEPARATOR_PROJECT_DIR")
    config_path = os.environ.get("SPEECH_SEPARATOR_CONFIG_PATH")

    if not project_dir:
        print("WARNINGS: Environment variable SPEECH_SEPARATOR_PROJECT_DIR not set. Using current working directory.")

    if not config_path:
        raise RuntimeError("Environment variable SPEECH_SEPARATOR_CONFIG_PATH must be set.")
...
# Module-level app for WSGI servers like gunicorn
try:
    app = get_app()
except Exception:
    app = None
...
def main():
    ...
    os.environ["SPEECH_SEPARATOR_PROJECT_DIR"] = args.project_dir if args.project_dir else os.getcwd()
    os.environ["SPEECH_SEPARATOR_CONFIG_PATH"] = args.config_path
