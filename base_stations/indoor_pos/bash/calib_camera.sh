...
if [[ "$OSTYPE" == "darwin"* ]]; then
  run_py_in_new_win_mac "python3 $PROJECT_DIR/examples/run_camera_calibrator.py"
elif is_raspberry_pi; then
  run_py_in_new_win_lxterminal "python3 $PROJECT_DIR/examples/run_camera_calibrator.py"
elif is_ubuntu; then
  run_py_in_new_win_gnome "python3 $PROJECT_DIR/examples/run_camera_calibrator.py"
else
  echo "Unsupported OS."
  exit 1
fi
