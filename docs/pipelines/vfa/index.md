...
# features instead of (or next to) the action labels: -a False -f True; the bases' keyframe_interval sets the rate
mmla vfa-sync -p pipelines/vfa-base -c pipelines/vfa-base/config.yml -sid <session-id> -nb 2 -a False -f True
```
...
The endpoint is stateless: one request, one set of synchronized frames. The synchronizer sends every frame set to it when its **Body & Gaze Features** toggle is on (`-f`) and writes the answer as one `vfa_features` event (see the [Database Reference](../../database.md#influxdb)); `Synchronizer.features_keypoints: false` keeps the skeletons out of the events, `Synchronizer.features_gaze: false` asks for the pose without the gazes (the request's `gaze=false`; the server's own `features.gaze` sets its default), and `Synchronizer.feature_zones_file` names a JSON file of zones. It also publishes each answer to the bases (`<session>/vfa/features` on MQTT): a base with **Graphics** on draws its own angle's boxes, tags, head yaws, skeletons and gaze lines on its live window, with a note of how old they are (the window runs at the camera's rate, the features at the bases' `keyframe_interval`). Tracking across frames, joining to the IPS positions and windowing into 10 s features belong to the analytics ([window features](#window-features-the-fusion-table)). The `features` block of the server config turns it off (`enabled: false`), picks the model and the confidence thresholds; the client helper is `request_frame_features` in `openmmla/services/vfa/requests.py`, and the example below sends still frames to it.
