...
`mmla asr-base -h` lists the toggles (`-s`, `-vad`, `-nr`, `-tr`, `-sp`, `-hsr`) and `-lang`, the language its speech is transcribed in. The speaker profiles of a host are listed, registered and deleted without a base's menu with `mmla asr-speakers`, which asks nothing:
