...
On a remote Mac the recorder runs FFmpeg in a Terminal window on that Mac's own screen and prints its output in your terminal as usual. macOS lets an app use the camera and the microphone only once someone has allowed it to, and anything started over SSH counts as `sshd`, which is never even asked: FFmpeg would wait for frames forever, and record silence from a microphone without a word. Terminal on the Mac's screen is an app that can be allowed. So that Mac needs someone logged in on its screen, and Terminal enabled under **System Settings → Privacy & Security → Camera** and **Microphone** (the first recording asks there, on the Mac itself). The device choice stays in your terminal, and Ctrl+C there, Stop and Stop All Hosts stop the FFmpeg in that window. Linux hosts and Raspberry Pis have no such permission: a user in the `video` and `audio` groups records over SSH directly.
