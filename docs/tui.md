...
The table is read again every time the tab comes into view, so a session created or recorded in the Launcher is there without a press on **Refresh** (which also reconnects to the databases). The Host selector opens on the machine the MongoDB address in System Settings names, as long as a saved SSH profile matches that address and answers (else on Local): the sessions live in that database, so they are listed without switching by hand, whichever host the Launcher is on. A host picked here stays, whatever the Launcher does.
