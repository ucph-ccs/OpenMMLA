...
!!! note "Layout version 4: re-fuse for the hands in body units (2026-09-24)"
    The fusion of 2026-09-24 (fixed before any label was read) centres the hand circle where the hand is, 0.33 shoulder widths past the wrist instead of 0.14, and makes every stored frame's gaze targets and hand distances again with it; it also follows each pupil's wrists in the image, in their own shoulder widths (see [the hand circle, remade and the hands in body units](../pipelines/vfa/index.md#window-features-the-fusion-table)). Layout version 4 reads the hands in those body units: per person the wrist speed in shoulder widths, the share of 1 s steps with the hands active and with them still (mask m_hands) and the share of frames with the hands of a body outside the group within a hand length of theirs (m_ohands); per pair the hand distance in shoulder widths, min and mean, and the share of frames with the hands within one hand length (m_hand), the shares of steps with one pupil active and the other still, both active and both still (m_steps), and, given at least three one-active steps, the share of them in which the still pupil's gaze was on the active one's hands (m_follow). The wrist speed and hand distances in frame widths are dropped: the camera's distance and field of view move them (the duplicate-skeleton gate still reads `hand_dist_mean`). The lag column `log_wrist_speed_sw_mean` replaces `log_wrist_speed_mean`. The body values are scaled [g], one scale for every camera. The same fusion takes `speech_ratio` and `silence_ratio` from the group microphone alone in a session with worn microphones beside it (four of the replayed sessions, and three windows of a fifth; see [how the fusion counts speech](../pipelines/asr.md#personal-microphones-and-energy-attribution)), as every test session has only that one. A table fused before the hand columns has none of them: their masks are off everywhere, and `data_checks.json` lists it under `fusion_check` as `hands: false`.
...
| person slots | 3 × 28 | present_ratio, log path, seen share, head-turn mean and spread / 90, log wrist speed (shoulder widths a second, + 0.01), the share of steps with the hands active, with them still, the share of frames with the hands of a body outside the group within a hand length, gaze switch rate, gaze on a partner's face, a partner's hands, someone else's face (`other_face`), someone else's hands (`other_hands`), own hands, the work area, elsewhere, out of frame (each over the readable share), readable share; masks m_ips, m_path, m_vfa, m_yaw, m_hands (the speed and the step shares), m_ohands (the hands of a body outside the group), m_gaze (partner, own, elsewhere, out of frame), m_other (the two other shares), m_wa (the work area) |
| pair slots | 3 × 22 | distance mean and min, face_any = max(ab, ba), co-seen share, hand distance min and mean in shoulder widths, the share of frames with the hands within a hand length, gaze distance, joint attention, joint-attention excess over the pair's own rate 20–40 s earlier, the shares of steps with one active and the other still, both active, both still, and the share of the one-active steps in which the still one's gaze was on the active one's hands; masks m_dist, m_face, m_covis, m_hand, m_gazepair, m_jexcess, m_steps (the step shares), m_follow (three or more one-active steps) |

Every pair value is symmetric in a and b. A value is scaled either as [g], a robust z-score over the training sessions' non-empty windows, or as [s], one within its own session. Per-camera units such as frame widths (the gaze distance), words and path use [s]; the hands, in shoulder widths, use [g]. The result is clipped to ±5.

**Pooled view.** Each person value becomes its (min, mean, max) over the slots that observed it, and each pair value the same over pairs. Masks, shares of slots observed and the group size are added, for 118 columns in four blocks: speech 10, space 18, body and gaze 89 (17 person values and 11 pair values, each as min, mean and max, and the 5 share columns), roster 1. For at most three persons, (min, mean, max) gives back the sorted values exactly. The pooled view therefore loses only which values belong to one person, and that binding is what the network's set encoders can add. The rule, LR, HGB, late fusion and PooledNet all read this view.
...
|---|---|---|
| T0 | nothing | 118 |
| T1c (causal) | 12 key columns at t−1 and t−2, and their mean over t−4..t | 154 |
| T2 (centred) | the same at t−1 and t+1, and their mean over t−2..t+2 | 154 |
...
- `wrist_speed`, `hand_dist_min` and `hand_dist_mean`, in frame widths: the camera's distance and field of view move them, and the body-normalised `wrist_speed_sw` and `hand_dist_sw_*` replace them (layout version 4);
...
- every coverage counter (`n_*`, `*_frames`, `*_frame_sets`, `*_cameras`, `*_hand_steps`). Camera count is a session fingerprint, so these are only masks and normalisers;
...
| Model | What it is |
|---|---|
| `r0` | The a-priori rule, version 3 (fixed 2026-09-24 for layout version 4, before any label was read): version 2 with its hands in body units. Layout version 4 has no hand distance in frame widths, so handing over is two pairs of hands within one hand length, `hand_dist_sw_min_min` ≤ 0.46 shoulder widths (a hand is a median 0.46 shoulder widths long). Version 2's 0.05 frame widths was about that: 0.50, 0.53 and 0.38 shoulder widths at the median seat of the 540p micro:bit, 1080p micro:bit and 1080p microscope cameras. Over the unlabelled windows of the 20 sessions in which a pupil pair has a hand distance, the clause fires in 44.7 % (69 %, 40 % and 46 % by setup) against 45.4 % for version 2 on the stored hands (72 %, 42 % and 39 %): the microscope cameras stand nearer, and in frame widths their pupils' hands had to be closer. Everything else is version 2, fixed 2026-09-23 for layout version 3, from the codebook's wording, before any label was read. Talk is speech ≥ 30 % with a change of speaker, or ≥ 5 words when not diarized ("members interact (talk, ...)"; version 1's thresholds). Look is ≥ 20 % of readable gaze on an in-group partner's face ("look at each other"; version 1's glance bound). Shared focus is joint attention ≥ 30 % above the pair's own rate 20–40 s earlier (`joint_attention_excess_max`), or hands close enough to hand over (version 3: within one hand length; version 2: ≤ 0.05 frame widths apart; "joint attention on the shared artifact", "handing over, working on one thing together"): sitting close is not a shared focus, so the raw share no longer counts. Watching is a pupil's share of gaze on a partner's hands ≥ 50 % (`partner_hands_max`; "one member following another's work on the shared artifact for most of the window counts, even in silence; a glance does not"). Any of the four makes an interaction. Shared focus or watching makes it collaborative, and so does an interaction with ≥ 40 % of gaze on the task: a partner's hands, own hands and the work area (`partner_hands_mean + own_hands_mean + work_area_mean`; "interact about the task", version 1's 0.4 with the shared artifact's region added). A role that sums columns keeps the first column's NaN (the condition is then false) and counts an added one as 0 where it is NaN. It gives hard labels only. Expected, and reported rather than tuned: social becomes rarer, and the pre-registered rare-social policy applies. The near-hands clause alone fires in 37 % of the unlabelled windows (39 % in version 2); it stays, because the codebook names handing over. |
| `r0-v1` | Version 1 of the rule (fixed 2026-09-23 with layout version 2), kept for the record. Talk, look (≥ 20 % on any other person's face: partner + other), or a shared focus (the raw joint attention ≥ 30 %, or near hands) make an interaction, and a shared focus or ≥ 40 % of gaze on hands (a partner's, someone else's or own) makes it collaborative. On a version 3 view it reproduces version 1 exactly, and on a table fused before the split the other shares are NaN and count 0. On a version 4 view, which has no hand distance in frame widths, its near hands are version 3's (within one hand length). Version 2 is no model of its own: on a version 4 view it would read the same hands and so be version 3. |
...
|---|---|
| `net` (default) | 14,057 |
| small configuration (used when a fold has fewer than 3,000 coded training windows; `--small`) | 6,961 |
| `net-pair` (pair encoder conditioned on both persons' states) | 15,209 |
| `net-notcn` (no temporal blocks) | 8,585 |
| `pooled-net` (the 118-column pooled view through the same fusion, temporal blocks and head) | 11,567 |

These are the counts of layout version 4 (person tokens of 28, pair tokens of 22, a pooled view of 118); version 3 gave 13,793, 6,785, 14,945, 8,321 and 10,367, and version 2 13,625, 6,673, 14,777, 8,153 and 9,767.
...
- tertiles fitted once on the dev sessions' windows, label-free, for speech, words, head turn, head-turn spread, gaze switches and hand distance (in hand lengths since 2026-09-24; a template frozen before holds the distance in frame widths, under another name, and the command asks for `--fit-bins`). Shares, of gaze or of the seconds the hands moved, go without a word.
...
Since 2026-09-24 the state says the hands in body units rather than in frame widths: per person "hands moving in 60% of the seconds, still in 20%" and, where it happened, "the hands of someone outside the group within a hand length of theirs in 10% of frames"; per pair "hands 0.9 hand lengths apart at the closest (medium), within one hand length in 40% of shared frames; one's hands moving while the other's were still in 50% of the seconds, the still one's gaze on the moving hands in 100% of those, both moving in 0%, both still in 25%" (the gaze part only with three or more such seconds). The pilot leaves out the hands' movement, which follows one camera's frames in time. The sensor note says that the cameras give whether each person's hands moved or stayed still from one second to the next, and how far apart hands were, in hand lengths. A table fused before has none of this and says "hand movement not measured" and "hands not measured".
