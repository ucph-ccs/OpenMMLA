...
**Manage**, next to Download, lists what the streams of the card have recorded on each capture host, newest day first, with the size of each file, the room left on the host's disk and the file a stream is writing now, and deletes a file, a day, or everything last written longer ago than a given age, each after a second press. Nothing on a capture host deletes these files by itself. **Keep recordings for** there sets `record_keep_days` of every stream of the card (also a field of each stream on the Config tab; `0` keeps them): a recording last written longer ago than that is deleted at its stream's next **Start** and at **Refresh** on the Streams tab, which say what they removed, and the Record column shows the time (`yes, 7 d`). The file a stream is writing is never deleted, by hand or by the keep time: a deleted file that is still being written keeps its room until the stream stops, and that take is lost, so stop the stream first. The folders a deletion leaves empty go with it, except today's. Copies fetched with Download are on this machine, where **Sessions → Delete Artifacts** removes a `streams-<date>` folder; the Stream Server keeps its own recordings for as long as its card says (below).
