...
- **A badge** sending `timestamped` packets and an **LSL** stream are stamped on the capture side, with the sender's clock.

Two things follow. Every machine that stamps (each base host, the stream server, a device that publishes over RTSP) must agree on the time: `timedatectl` on Linux (`System clock synchronized: yes`; `sudo timedatectl set-ntp true` when not) and `sntp time.apple.com` on a Mac say by how much a clock is off; a Raspberry Pi without internet needs an NTP server on the network (`chrony` on the stream server's host with an `allow` line for the subnet, `NTP=<stream-server>` in the Pi's `/etc/systemd/timesyncd.conf`). And every stream carries a constant delay of its own that nothing measures for you. It is well under a second, which the pipelines' windows absorb (the VFA synchronizer matches angles within `match_tolerance`, ASR buckets are three seconds, the fusion table ten), so a session's modalities agree to the second as they are. For claims that need less than that, measure each stream once and write the result into `stream_kwargs.timestamp_offset` of its base's config, which the base adds to every stamp (a delay of 0.35 s is `timestamp_offset: -0.35`). The base logs where its stamps come from when the first frame arrives (`Stream stamps come from ...`), and leaves the ones that already carry the capture side's clock alone (the registered stream start, a badge's packet header, LSL), so measure on the source you will run with, and note that the value is shared by every base of the config (ASR: of the base type): bases on streams with different delays belong in different configs.
...
2. Film that screen through the stream with `store: true`, for a minute: the VFA base saves a keyframe every `keyframe_interval` seconds into its `real-time/` folder, the IPS base one frame a second.
3. For a few saved keyframes, subtract the unix time visible in the picture from the time in the file's name: that difference is the stream's delay, and its negative the offset. The keyframes of a `stream` source are evenly spaced, so five frames settle it.
...
The offset is in every stamp from then on, including the `vfa_features` events and the fusion table, and in the analyze mode's replay of the saved frames.
...
**Manage**, in the Recordings row of the Streams tab, lists what the streams of the card have recorded on each capture host, newest day first, with the size of each file, the room left on the host's disk and the file a stream is writing now. It copies nothing to this machine: a session's part of the recordings is taken with **Sessions → Export Streams**, [below](#a-sessions-part-sessions-export-streams), which takes the Stream Server's copy along with it. (The Collection card's own Download fetches one *session* of its recorders, which is a different thing: a Collection recorder is started by a session and ends with it, so its files are filed under that session, while a stream is started on its own and shared.) A camera or microphone can be either streamed or recorded by the Collection card, not both at once; `record: true` is the way to get both.

Manage deletes on the capture hosts: **Delete File** the selected recording, **Delete Day** every recording of the selected recording's day (the **Day** column) on its host, **Delete Older Than** every recording last written longer ago than the age beside it, on every host. Each takes a second press; the first says what would go and how much room it frees, and Delete Day names the day and the host. Nothing on a capture host deletes these files by itself. **Keep recordings for** there sets `record_keep_days` of every stream of the card (also a field of each stream on the Config tab; `0` keeps them): a recording last written longer ago than that is deleted at its stream's next **Start** and at **Refresh** on the Streams tab, which say what they removed, or at once with **Delete Expired**, and the Record column shows the time (`yes, 7 d`). The file a stream is writing is never deleted, by hand or by the keep time: a deleted file that is still being written keeps its room until the stream stops, and that take is lost, so stop the stream first. The folders a deletion leaves empty go with it, except today's. Deleting on a capture host leaves what Export Streams copied to this machine alone: a session's part, under `artifacts/<session>/streams/`, goes with that session's **Delete Artifacts**. The Stream Server keeps its own recordings for as long as its card says (below).

### On the server

While `record` under `pathDefaults` is on in `mediamtx.yml` (the **Server-side recording** switch on the Stream Server card's Config tab; on by default), MediaMTX writes every published path to `artifacts/streams/server/<app>/<name>/<YYYY-MM-DD_HH-MM-SS-ffffff>.mp4` on the Stream Server's host, in ten-minute fMP4 segments (see [Run it](#run-it) for how both run modes land there). Segment names are the server's clock (the publisher's only for a path with `useAbsoluteTimestamp` on, see [Timestamps](#timestamps)), so treat them as an archive of what arrived rather than as a capture-side recording. The server knows nothing of sessions: it records a path from the moment something is published until the publisher stops, whether or not a session runs. A segment is deleted `recordDeleteAfter` after it began, by MediaMTX itself: three days in the shipped file (**Keep recordings for** on the card's Config tab; `0s` keeps everything), so a session's footage has to be exported before then. The **Sessions** table says until when, in its **Recordings until** column, and the card's **Recordings** tab shows what the server holds, path by path with sizes and free disk on `artifacts/streams/server/`, and deletes a path or everything older than a given age through the API. An empty table has three readings and the tab says which, by asking the server what it is set to record and what is being published to it: recording switched off, nothing published, or a path publishing right now that the server has written nothing for. The last is a recorder that cannot write — usually a container whose record folder was renamed or removed under it, which keeps the old folder open and can create nothing in it until a new container is made (Stop, then Start); `docker compose -f docker/docker-compose.infra.yml logs --tail 50 mediamtx` on the server's host says so in as many words. The playback server returns any time range of a path as one file:
